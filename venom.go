...
func (v *Venom) GetExecutorRunner(ctx context.Context, ts TestStep, h H) (context.Context, ExecutorRunner, error) {
	...
	Info(ctx, "looking for executor %v", name)
	...
	ex, err := v.GetExecutor(name)
	if err != nil {
		return ctx, nil, fmt.Errorf("%v", err)
	}
...
func (v *Venom) getUserExecutorFilesPath(vars map[string]interface{}) (filePaths []string, err error) {
	var libpaths []string
	if v.LibDir != "" {
		p := strings.Split(v.LibDir, string(os.PathListSeparator))
		libpaths = append(libpaths, p...)
	}
	libpaths = append(libpaths, path.Join(fmt.Sprintf("%v", vars["venom.testsuite.workdir"]), "lib"))
	...
	Info(context.Background(), "%v potential user executors", len(filePaths))
	if len(filePaths) == 0 {
		return []string{}, nil
	}
...
func (v *Venom) RegisterUserExecutors(vars map[string]interface{}) error {
	paths, err := v.getUserExecutorFilesPath(vars)
	if err != nil {
		return err
	}

	v.executorPaths = paths
...
func (v *Venom) GetExecutor(name string) (Executor, error) {
	executor := v.executorsUser[name]
	log.Debugf("looking at %v executor", name)
	if executor == nil {
		log.Debugf("NOT having it so we are trying to parse %v executor", name)
		for _, f := range v.executorPaths {
			log.Debugf("looking for %v", f)
			if strings.HasSuffix(f, fmt.Sprintf("%s.yml", name)) || strings.HasSuffix(f, fmt.Sprintf("%s.yaml", name)) {
				log.Debugf("matches name")
				ux := UserExecutor{Filename: f}
				btes, err1 := os.ReadFile(f)
				if err1 != nil {
					return nil, errors.Wrapf(err1, "unable to read file %q", f)
				}
				if err := yaml.Unmarshal(btes, &ux); err != nil {
					return nil, errors.Wrapf(err, "unable to parse file %q", f)
				}
				executor = ux
				v.executorsUser[name] = ux
			}
		}
	}
	return executor, nil
